        ...
        <div className="pt-4">
          ...
          <div className="space-y-4">
            {request.results.map((result, index) => {
              const { text: caseTextColor, bg: caseBgColor } = getCaseCodeColors(result.caseCode);
              ...
              return (
                <div key={index} className="mb-4">
                  <div className="flex items-center justify-between text-sm text-gray-600">
                    ...
                    {/* Case Code Badge */}
                    <span
                      className={`px-2 py-1 text-xs font-medium rounded-md ${caseTextColor} ${caseBgColor}`}
                    >
                      {result.caseCode}
                    </span>
                  </div>
                  <p className="mt-2 text-sm whitespace-pre-line leading-loose">{result.description}</p>
                </div>
              );
            })}
          </div>
